...
def Stch(setch, mybaudrate = 19200):
	...
	#以下でやってること
	'''
	    ENWRコマンドを使用して、設定の書き込みエンエーブルを有効にします。これにより、設定の変更が可能になります。
        Stchコマンドを使用して、指定された通信チャンネルを設定します。引数として指定されたチャンネル番号を、文字列形式でIM920slモジュールに送信します。
        DSWRコマンドを使用して、設定の書き込みを確定させます。これにより、実際に設定がIM920slモジュールに反映されます。
	'''
	com = setSerial(mybaudrate)
	com.flushInput()
	com.write(b'ENWR' + b'\r\n')
	com.flushOutput()
	com.readline()
	setch = input("チャンネル数何にする？ 00~15で選んでね")
...
def Strt(setspeed, mybaudrate = 19200):
	...
	com.flushOutput()
	com.readline()
	setspeed = input("チャンネル数何にする？ 1 or 2で選んで。 1:高速通信、2:長距離通信")
